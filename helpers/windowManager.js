class WindowManager {
    ...
    centerWindow(window) {
        ...
        Logger.log(Logger.expandFrame(sFrame, Object.keys({sFrame})[0]));    
        Logger.log(Logger.expandFrame(wFrame, Object.keys({wFrame})[0]));    
    ...
    growFrame(x, y, width, height, window = Window.focused()) {
        ...
        if (width)
            newFrame.width = Math.min(x ? Math.abs(frame.x) + frame.width : sFrame.width - frame.x, frame.width + (width - (Math.abs(x) - Math.abs(frame.x - newFrame.x))));
        if (height)
            newFrame.height = Math.min(y ? Math.abs(frame.y) + frame.height : sFrame.height - frame.y, frame.height + (height - (Math.abs(y) - Math.abs(frame.y - newFrame.y))));
